...
undo日志主要支撑两个主要功能：事务回滚和MVCC。

- 事务回滚：在事务进行过程中，当对数据页进行改动时，将旧版的数据记录下来。当需要回滚时，通过undo日志中的数据将数据恢复。
- MVCC：undo日志会保存数据的多份历史数据，通过readview和undo日志结合，就可以读取到正确的数据。

**事务id**

事务包含只读事务和读写事务。对于只读事务，只有在对临时表进行增删改操作的时候才会分配事务id；对于读写事务，在对表进行增删改操作的时候才会分配事务id。

**trx_id**

聚簇索引中的每一条记录都存在trx_id的隐藏列，当我们对一条记录进行增删改操作的时候，会将这次改动的事务的事务id写到trx_id列中

#### undo日志类型

undo日志一般分为两大类：`TRX_UNDO_INSERT`和`TRX_UNDO_UPDATE`。其中`TRX_UNDO_INSERT`一般由insert和update语句产生，`TRX_UNDO_UPDATE`一般由delete和update语句产生。

- 当插入一条记录时，在undo日志中记录插入记录的主键信息，回滚时通过主键信息将相关记录删除
- 当删除一条记录时，对原记录做删除标记，同时在undo日志中记录原记录的主键和二级索引等信息，回滚时将数据重新更新为旧数据
- 当更新一条记录时，在undo日志中记录主键、二级索引以及被更新列的更新的信息，回滚时将数据重新更新为旧数据。update类型还可能变成先删除再插入的情况，和插入删除日志类似。

#### 回滚段

**undo页：**首先对于undo日志记录都是记录在物理页上，按上述两个大类划分，两个大类的undo日志记录不可以混合存放在同一个页面内。

**undo页面链表：**一个事务产生的undo日志可能很多，当一个物理页存储不下时，就需要将多个物理页以链表的形式连接起来，形成undo页面链表，同样一个链表中的页面只能属于同一个大类。除此之外，对于临时表和普通表的undo日志也是在不同的链表中存放的，因此一个事务在执行中最多可能产生4个undo页面链表，普通表和临时表分别都有一个insert链表和一个update链表。

**undo段：**每时刻一个undo段都只会被一个事务独占，一个undo页面链表就对应一个undo段。undo段的信息记录在该undo段对应的undo页面链表中第一个页面内。

**回滚段：**为了管理和使用undo段，使用回滚段(rollback segment)。在回滚段中存在1024个slot，每个slot对应一个undo段。在数据库系统中每个Undo Tablespace最多会包含128个回滚段，比如在8.0+的版本中，系统默认存在两个Undo Tablespace文件，命名为`undo_001`和`undo_002`，每个文件都包含128个回滚段。

各结构关系如下图：

- 在undo_001的文件中，3号页中保存了128个回滚段的信息，比如回滚段0在4号页，回滚段1在5号页，以此类推
- 对于4~132号页，每个页都是回滚段的信息，每个回滚段包含1024个slot，每个slot也就是一个undo段，指向一个undo页面，也即一个undo页面链表的第一个页面
- 在内存中使用`trx_rseg_t`结构来保存回滚段的信息，其中重要的是包含了4个链表，用于管理和使用slot，也即undo段；使用`trx_undo_t`结构体来保存undo段的相关信息

![undo_segment](https://qraffa-1304595678.cos.ap-guangzhou.myqcloud.com/img/undo_segment.svg)

##### 事务分配undo页面链表

1. 在事务执行过程中，当需要进行增删改操作时，也就是需要写undo日志了。那么首先需要分配回滚段(rollback segment)。分配回滚段的方式采用`round-robin(循环使用)`，即从0号回滚段，下一次从33号，34号依次类推（0号和33~127号给普通表，1~32给临时表）。
2. 分配到回滚段后，就需要分配undo段了，首先会根据操作类型从两种cached链表中查找能够复用的undo段。update类型就找update_undo_cached链表，insert类型insert_undo_cached链表
   1. 复用条件：一个undo段只占用了一个undo页面，并且该页面的使用量小于页大小的3/4
3. 如果没有找可复用的undo段，那么就需要创建新的undo段，也就是找该回滚段中空闲的slot
4. 找到undo段后，如果是新创建的，那么还需要为其创建新的undo页面作为链表的第一个页面，然后更新相关信息
5. 完成上述后，就可以向undo页面中写入undo日志了

#### 崩溃恢复

崩溃恢复时，数据库通过redo日志将内存和文件恢复到崩溃前的状态，然后扫描所有的回滚段，以及回滚段中的undo段，通过检查undo段的状态，如果是TRX_UNDO_ACTIVE，则需要通过该undo段的相关日志进行回滚。